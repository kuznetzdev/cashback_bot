...
async def handle_command(
    ...
    if name == "open_help":
        return workflow_screens.result_with_screen(user=user, state=state, screen=workflow_screens.help_screen())
    if name == "cancel_flow":
        return workflow_screens.result_with_screen(user=user, state=WorkflowState(), screen=workflow_screens.home_screen())
    if name == "open_top":
    ...
    if name == "set_language":
        updated_user = await deps.change_language_use_case.execute(user_id=user.id, language=str(command.payload["code"]))
        return workflow_screens.result_with_screen(user=updated_user, state=state, screen=workflow_screens.settings_screen(updated_user))
    ...
    if name == "open_history":
        ...
        return workflow_screens.result_with_screen(user=user, state=state, screen=workflow_screens.history_screen(logs))
    return None
